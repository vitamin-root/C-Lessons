...
namespace _2_5_JAGGET_ARRAY
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < numbers.Length; i++)// 4 количество строк
            {
                for (int j = 0; j < numbers[i].Length; j++)// numbers[i].Length - количество элементов в каждой строке
                {
                    Console.Write($"{numbers[i][j],8}");
                }
                Console.WriteLine();
            }
            Console.WriteLine();
            ...
            for (int i = 0; i < numbers.Length; i++)
            {
                if (numbers[i] != null)
                {
                    ...
                    Array.Copy(numbers[i], copy[i], numbers[i].Length);// копия каждой строки
                }
            }
